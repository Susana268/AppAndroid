package com.sgonzalez.myappbdsqlite.basededatos;

import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;

import com.sgonzalez.myappbdsqlite.complementos.ConstantesSQL;
import com.sgonzalez.myappbdsqlite.complementos.ConsultasDAO;
...
public class BebidaDAO implements ConsultasDAO {
    ...
    public boolean insertarBebida(BebidaVO bvo, Context context) {
        boolean resultado= false; //validar contexto null
        ...
        //Paso 2 Instanciar un objeto que nos permitira realizar la accion de escribir, registrar o insertar datos
    ...
    public Cursor buscarIdBebida(BebidaVO bvo, Context context) {
        ConectorSQLite conectorSQLite=new ConectorSQLite(context,ConstantesSQL.BD_BEBIDA,null,ConstantesSQL.VERSION);
        //Declarar el objeto que nos permitira leer la informacion
        SQLiteDatabase database= conectorSQLite.getReadableDatabase();
        // Crear el objeto de tipo cursor este me permite solicitar informacion a la BD y luego leerla y obtenerla
        Cursor cursor = null;
        // Este arreglo permite identificar un paramtero para consultar
        String[] parametro = {bvo.getCodBebida().toString()};
        try{
            String consulta = "SELECT "+
                              ConstantesSQL.CAMPO_CODIGO+", "+
                              ConstantesSQL.CAMPO_NOMBRE+", "+
                              ConstantesSQL.CAMPO_SABOR+", "+
                              ConstantesSQL.CAMPO_PRESENTACION+", "+
                              ConstantesSQL.CAMPO_TIPO+", "+
                              ConstantesSQL.CAMPO_PRECIO+" FROM "+
                              ConstantesSQL.TBL_BEBIDA+
                              " WHERE " + ConstantesSQL.CAMPO_CODIGO+ " = ?";
            cursor = database.rawQuery(consulta,parametro);
            cursor.moveToFirst();
    ...
    public boolean actualizarBebida(BebidaVO bvo, Context context) {
        ...
        ConectorSQLite contectorSQLite = new ConectorSQLite(context,ConstantesSQL.BD_BEBIDA,null,ConstantesSQL.VERSION);
        SQLiteDatabase database = contectorSQLite.getWritableDatabase();
        try{
